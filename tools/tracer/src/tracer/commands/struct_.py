"""`trace struct` — AST-structural search via ast-grep, with rich context.

Same enrichment as grep (per-file complexity, nearest doc, git activity).
File named struct_.py to avoid shadowing stdlib's struct module.
"""

from __future__ import annotations

import json
import subprocess
from pathlib import Path

import click

from tracer.deps import require_dependencies
from tracer.enrich import file_complexity, git_context, nearest_doc
from tracer.repo_context import repo_context


def _ast_grep(pattern: str, lang: str, path: str) -> list[dict]:
    result = subprocess.run(
        ["sg", "run", "-p", pattern, "-l", lang, "--json", path],
        capture_output=True,
        text=True,
        check=False,
        timeout=60,
    )
    if not result.stdout.strip():
        return []
    try:
        data = json.loads(result.stdout)
    except json.JSONDecodeError:
        return []
    matches: list[dict] = []
    for entry in data:
        matches.append(
            {
                "file": entry["file"],
                "line": entry["range"]["start"]["line"] + 1,
                "snippet": entry["text"],
            }
        )
    return matches


@click.command()
@click.argument("pattern")
@click.option("-l", "--lang", required=True, help="Language for AST parsing (e.g. python, tsx, php)")
@click.option("--path", default=".", help="Path to search")
@click.option("--json", "as_json", is_flag=True)
def command(pattern: str, lang: str, path: str, as_json: bool) -> None:
    """Structural (AST) search via ast-grep, with per-match enrichment."""
    require_dependencies()
    matches = _ast_grep(pattern, lang, path)

    file_cache: dict[str, dict] = {}
    enriched: list[dict] = []
    for m in matches:
        fpath = m["file"]
        if fpath not in file_cache:
            file_cache[fpath] = {
                "file_complexity": file_complexity(fpath),
                "nearest_doc": nearest_doc(fpath),
                "git": git_context(fpath),
            }
        enriched.append({**m, **file_cache[fpath]})

    output = {
        "pattern": pattern,
        "lang": lang,
        "matches": enriched,
        "match_count": len(enriched),
        "files_matched": len(file_cache),
        "repo_context": repo_context(path),
    }

    if as_json:
        click.echo(json.dumps(output, indent=2))
        return

    if not enriched:
        click.echo("(no matches)")
        return

    current_file: str | None = None
    for m in enriched:
        if m["file"] != current_file:
            current_file = m["file"]
            ccn = m["file_complexity"]
            doc = m["nearest_doc"] or "(no doc)"
            git = m["git"]
            click.echo("")
            click.echo(f"{current_file}  [ccn={ccn['ccn_total']} {ccn['rank']}, last={git['last_modified']}, doc={doc}]")
        click.echo(f"  L{m['line']:<5} {m['snippet']}")

    ctx = output["repo_context"]
    click.echo("")
    click.echo(f"matches={output['match_count']} files={output['files_matched']} repo_p95={ctx['complexity_p95']}")
