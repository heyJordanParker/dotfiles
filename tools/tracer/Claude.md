...
- `context [<path>] [--directory] [--offset N] [--limit N] [--no-record]` — session-start primer (no args) or single-file enrichment (path arg). `--offset`/`--limit` are the read tool's line-range parameters; when set, the file's read is recorded as that 1-based inclusive span (absent → whole file) so the session log accumulates per-file read coverage (the union of every range read), surfaced by `docs status`. `--no-record` renders the shoulder but records no read — the enrich hook sets it for Edit/Write so an edit gets the file's architectural shoulder without masquerading as a read; read coverage stays a function of genuine reads alone
...
- `docs load <path> [--source <s>] [--triggering-tool <t>] [--triggering-command <c>]` — hook-facing alias forwarding to path-mode with the `--source` default flipped to `trace_docs_load`. Same `{ docs, doc_count, already_loaded? }` shape, same behavior. `inject_docs.py` invokes path-mode directly
...
- `docs status [<path>]` — agent-facing "what do I have right now?" query against the session log. No path → the full session manifest with per-entry source attribution, a `by_source` count breakdown, and per-file read coverage (`total_lines`, `lines_read`, `read_fraction` — the fraction of the file's lines read this session, `0.0` for a doc-injected file never read). With a path → the ancestor chain partitioned into `loaded` (with source) and `not_loaded` so the agent can immediately tell whether a path's rules are in context. Pure read; never records.
...
- **`sessions/<session_id>/<agent_id>/`** (in `.tracer-cache/` at the repo root, alongside the other two namespaces) — the session-context log. `events.jsonl` is the append-only event log (one JSON object per line); `view.json` is the materialized projection (`emitted: canonical path → sha256:<hex>`, plus `coverage: canonical path → { total_lines, read: [[start,end],…] }` — the union of every line range read for each file, the per-file read-coverage accumulator `docs status` surfaces); `.lock` carries the flock held across read + append + materialize so concurrent same-session invocations are serialized. `session_id` is resolved from `AGENT_SESSION_ID` / `CODEX_THREAD_ID` / `CLAUDE_CODE_SESSION_ID` via `nested_memory::session_id()` — the harness-neutral carrier first, then the innermost harness session before the outer; `agent_id` is resolved from `TRACER_AGENT_ID` and defaults to the literal string `root` when absent. The log no-ops entirely when the session id is absent OR when no repo root is resolvable from cwd, keeping standalone tracer use valid in both cases. Session context that spans repos is not preserved across repos — accepted trade-off for one storage location and one cleanup story across all three namespaces. This is the single owner of session-context state — no other module persists session dedupe or related data. Stopped subagents are archived to `sessions/<session_id>/archived/<agent_id>/` by the `archive_subagent_log.py` harness hook; the log's read path falls back to the archived directory when the active one is absent, so queries against a stopped subagent's log keep returning the same data. Writes always target the active directory — never the archived one. A `ContextPrimeDrift` event records divergence between the deterministic context primer's predicted set and the observed set Claude Code actually injected, supplied to `trace context prime --observed-from <path>` (`-` for stdin); on drift the view's `emitted` map is reconciled to observed paths and their hook-supplied content hashes while the append-only `events.jsonl` preserves the diff payload.
...
- Never walk the whole tree before filtering for repo-wide enumeration — use `repo_files::tracked_files` (git ls-files, repo-root-relative) or its absolute-path peer `tracked_paths`, or `walk_files` with dir-level `SKIP_DIRS` pruning, never an unbounded recursive walk that descends `node_modules` / `vendor` / worktrees
- Never run a private `git ls-files` to enumerate repo files — `repo_files` is the single source of truth for the deletion policy (a path in git's index but absent from disk is excluded). Every file-listing command routes through `tracked_files` / `tracked_paths` so `list` / `tree` / `find` / `glob` / the architecture graph agree with the working tree, never with git's stale index. A second enumerator re-introduces the divergence this collapses
...
│   ├── repo_files.rs                   the single file enumerator: `tracked_files` (git ls-files, repo-root-relative) + `tracked_paths` (same set, absolute) + `walk_files` (SKIP_DIRS fallback); owns the deletion policy (index-but-not-on-disk excluded) every listing command shares
