...
Each section is the rule (when + why), the contract (data-attribute + var schema), and the minimal CSS expressing the unique mechanism. Boilerplate (`will-change`, `prefers-reduced-motion`, `transition-property` for non-mechanism properties, asymmetric open/close timing logic) lives in [motion.md](motion.md) — apply globally.
