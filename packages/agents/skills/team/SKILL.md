...
- **Never forward a teammate's ranking or recommendation.** Teammates return facts. You eliminate bad options, you finish the research, you rank with /pcc, you recommend one — all in your own voice. Full rules in /subagents ("You do the ranking. Subagents do not.").

## You do the ranking. Teammates do not.

A teammate sees a slice — its task, its DoD, the files it touched.
You see the team's whole arc and the architect's prior calls. When a
teammate sends back "Option 1 is best", that ranking was made
without the context you hold. Pasting it through is how the
architect ends up reading a confident-sounding call made without the
load-bearing context.

So a teammate's recommendation is one of its findings. Strip the
recommendation. Keep the facts. Then do the ranking yourself:

1. **Run the research until it is done.** The first message back is
   not the research; it is the first message. SendMessage to every
   teammate whose work came back thin. Stop when no gap is left.
2. **Eliminate.** Drop every option that breaks a standard,
   convention, or project rule. You do this — not a teammate.
3. **Rank the survivors with /pcc.** Your /pcc, informed by
   everything a teammate could not see.
4. **Recommend one.** From your ranking. In your voice.

Banned phrases — same list as /subagents, plus the teammate
variants: "the teammate recommends", "per the teammate".

Research-teammate prompts close with this paragraph, verbatim:

> Close every research gap. SendMessage every gap you find. Do not
> stop to ask, do not deliver half-finished work. Every code claim
> must come from a read, not a guess. Findings only — no scope
> changes, no ranking, no recommendation.
...
claim. Full rules and failure-mode names live in /subagents
...
- **Forwarding a teammate's ranking** — forwarded recommendation. Strip it. Re-rank yourself with /pcc
- **Accepting first-message findings as the research** — first-batch-as-final. SendMessage every gap until nothing is left to investigate
...
- **Teammate returned a ranking or "best option"?** → forwarded recommendation. Strip it. Re-rank yourself with /pcc
- **First message of findings back?** → first-batch-as-final. SendMessage every gap until nothing is left
- **About to write "the teammate recommends" or "per the research"?** → stop. That sentence is the failure
