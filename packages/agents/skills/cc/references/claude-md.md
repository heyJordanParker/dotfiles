...
## Proactive Offers (architectural decisions)
...
**What to Exclude:**
- **Execution narrative** — the story of how the code reached its current state: what was migrated, moved, tried, renamed, or decided this session. The doc owns the destination; git owns the journey. Test: would a reader arriving cold, with no memory of this session, act on this line? If it reads as something that *changed* rather than something that *is*, cut it.
  - Bad: "Agents were moved from `packages/claude/agents/` to `packages/agents/agents/`; the old paths now symlink across" — narrates the move
  - Good: "Agents source of truth: `packages/agents/agents/<name>.md`; Claude reads them via the `claude/agents → ../agents/agents` symlink" — states the durable structure
- Session-specific context and temporary decisions
