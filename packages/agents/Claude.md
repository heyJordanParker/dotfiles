...
- `effort` is one field for both Harnesses: `low`, `medium`, `high`, or `max`, read natively by Claude and passed verbatim to codex by `lib/codex_run.py`, which accepts the same four. Omitting it runs the default there; any other word fails the run.
