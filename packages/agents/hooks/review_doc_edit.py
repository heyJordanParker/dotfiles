...
SYSTEM_PROMPT = (
    ...
    "rule it owns, a Fact outside a plain listicle sentence, or a trigger section in a "
    "body when the frontmatter description is the only trigger. The /cc labels are sane "
    "defaults, not the whole set: a labeled line or a labeled indented block under its "
    "own Rule is that Rule's supporting shape whatever the label word is, and is NEVER "
    "this finding. Evidence: quote the malformed block AND name the shape it should "
    "have.\n"
