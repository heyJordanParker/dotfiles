...
JSON_SCHEMA='{"type":"object","properties":{"type":{"type":"string","enum":["question","approval","instructions"]},"instructions":{"type":"array","items":{"type":"string"}},"skills":{"type":"array","items":{"type":"string"}},"proposal_expected":{"type":"boolean"},"approach":{"type":"string","enum":["solo","default","team"]},"sequential":{"type":"boolean"},"finalize":{"type":"boolean"},"session_notes":{"type":"array","items":{"type":"string"}}},"required":["type"]}'
...
6. All user instructions contain subtleties and nuance — preserve ordering language, execution context, autonomy cues, and boundary conditions verbatim. Never flatten, summarize, or strip nuance from extracted instructions.
...
8. Set \"approach\" based on user signals. \"solo\" when user wants the agent to work alone (\"do this yourself\", \"don't spawn agents\", \"read it yourself\"). \"team\" when user wants persistent teammates (\"get a team\", \"dispatch agents\", uses /subagents). \"default\" when neither signal is present. Only change from the current approach (${CURRENT_APPROACH}) on clear intent shift.
...
Otherwise return: {\"type\": \"instructions\", \"instructions\": [\"each specific instruction — preserve all nuance\"], \"skills\": [\"/detected-skills-being-invoked\"], \"proposal_expected\": true/false, \"approach\": \"solo/default/team\", \"sequential\": true/false, \"finalize\": true/false, \"session_notes\": [\"full updated list if changes, empty if no changes\"]}
...
jq -n \
    ...
    '{approach: $approach, finalize: $finalize, notes: $notes, validation_phase: 0}' \
